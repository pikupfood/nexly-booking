        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: '14px' }}>
          {SERVICES.map((s, i) => (
            <Link key={s.href} href={s.href} style={{ textDecoration: 'none' }}>
              <div className={`service-card anim-fadeup-${i + 2}`} style={{ background: '#fff', border: '1px solid #e8e6e1', borderRadius: '12px', padding: '28px 22px', cursor: 'pointer', transition: 'border-color 0.2s, box-shadow 0.2s', height: '100%' }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = s.color; e.currentTarget.style.boxShadow = `0 4px 20px ${s.color}18` }}
                ...
                <div style={{ fontSize: '13px', color: s.color }}>Réserver →</div>
              </div>
            </Link>
          ))}
        </div>
