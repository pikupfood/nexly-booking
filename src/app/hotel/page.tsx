...
const IS: any = { width:'100%', padding:'11px 14px', background:'#fff', border:'1px solid #d8d5d0', borderRadius:'8px', color:'#1a1a1a', fontSize:'14px', fontFamily:"'DM Sans', system-ui, sans-serif", fontWeight:300, outline:'none', transition:'border-color 0.2s', boxSizing:'border-box' }
...
export function PageWrapper({ children, title, subtitle, back }: any) {
  return (
    <main style={{ minHeight:'100vh', background:'#fafaf9' }}>
      <nav style={{ padding:'16px 32px', display:'flex', alignItems:'center', gap:'16px', borderBottom:'1px solid #e8e6e1', background:'#fff', position:'sticky', top:0, zIndex:50 }}>
        ...
        <div>
          ...
          {subtitle && <div style={{ fontSize:'10px', letterSpacing:'0.12em', color:'#9a9690', textTransform:'uppercase' }}>{subtitle}</div>}
        </div>
      </nav>
      <div style={{ maxWidth:'640px', margin:'0 auto', padding:'40px 24px 80px' }}>{children}</div>
...
export function Steps({ current, steps }: { current:number; steps:string[] }) {
  return (
    <div style={{ display:'flex', alignItems:'center', marginBottom:'40px', paddingTop:'8px' }}>
      {steps.map((s, i) => (
        <div key={i} style={{ display:'flex', alignItems:'center', flex:1 }}>
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', flex:1 }}>
            <div style={{ width:'26px', height:'26px', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'12px', fontWeight:500, background: current>i+1?'#1a1a1a':current===i+1?'transparent':'transparent', color: current>i+1?'#fff':current===i+1?'#1a1a1a':'#c0bdb8', border:`1.5px solid ${current>=i+1?'#1a1a1a':'#d8d5d0'}`, transition:'all 0.2s' }}>
              {current>i+1?'✓':i+1}
            </div>
            <div style={{ fontSize:'11px', color:current===i+1?'#1a1a1a':'#9a9690', marginTop:'5px', fontWeight:current===i+1?500:400 }}>{s}</div>
          </div>
          {i<steps.length-1 && <div style={{ flex:1, height:'1px', background:current>i+1?'#1a1a1a20':'#e8e6e1', marginBottom:'18px' }} />}
        </div>
      ))}
    </div>
  )
...
export function NavBtns({ onBack, onNext, nextLabel, nextDisabled }: any) {
  return (
    <div style={{ display:'flex', gap:'10px', marginTop:'32px' }}>
      {onBack && <button className="btn-secondary" onClick={onBack} style={{ padding:'11px 20px', borderRadius:'8px', border:'1px solid #d8d5d0', background:'transparent', color:'#6b6760', cursor:'pointer', fontSize:'13px' }}>← Retour</button>}
      {onNext && <button onClick={onNext} disabled={nextDisabled} style={{ flex:1, padding:'12px 20px', background:nextDisabled?'#e8e6e1':'#1a1a1a', color:nextDisabled?'#a0a0a0':'white', border:'none', borderRadius:'8px', cursor:nextDisabled?'not-allowed':'pointer', fontSize:'13px', fontWeight:500 }}>
        {nextLabel||'Continuer →'}
      </button>}
    </div>
  )
...
export function ConfirmBox({ children }: any) {
  return <div style={{ background:'#f5f5f3', border:'1px solid #e0ddd8', borderRadius:'10px', padding:'18px 20px', marginBottom:'24px' }}>{children}</div>
}
  ...
  return (
    <main style={{ minHeight:'100vh', background:'#fafaf9', display:'flex', alignItems:'center', justifyContent:'center', padding:'24px' }}>
      <div className="anim-fadeup" style={{ textAlign:'center', maxWidth:'400px', width:'100%' }}>
        <div style={{ width:'56px', height:'56px', background:'#1a1a1a', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'22px', margin:'0 auto 24px', color:'white' }}>✓</div>
        ...
        <Link href="/" style={{ display:'inline-block', padding:'12px 28px', background:'#1a1a1a', color:'#fff', borderRadius:'8px', fontSize:'14px' }}>Retour à l'accueil</Link>
...
export default function HotelBookingPage() {
  ...
  useEffect(() => {
    supabase.from('room_types').select('*')
      ...
      .order('base_price')
      .then(({ data }) => { setRoomTypes(data || []); setLoading(false) })
  }, [])
  ...
  const handleSubmit = async () => {
    ...
    const { data: guest } = await supabase.from('guests').insert([{
      ...
      email: client.email, phone: client.phone, tenant_id: TENANT_ID,
    }]).select().single()
    ...
    const { data: res } = await supabase.from('reservations').insert([{
      ...
      channel: 'direct', status: 'confirmed', tenant_id: TENANT_ID,
    }]).select().single()
    ...
    <PageWrapper title="Hébergement" subtitle="Réservation de chambre" back="/">
      ...
      {step===1 && (
        <div className="anim-fadeup">
          ...
          {roomTypes.length === 0 && <p style={{ color:'#8a8680', fontSize:'14px' }}>Aucune chambre disponible.</p>}
              ...
              <div key={rt.id} onClick={() => setSelectedType(rt)} style={{ padding:'18px 20px', borderRadius:'10px', cursor:'pointer', background:'#fff', border:`1.5px solid ${selectedType?.id===rt.id?'#1a1a1a':'#e8e6e1'}`, transition:'border-color 0.15s' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                  <div>
                    ...
                    {rt.amenities?.length>0 && <div style={{ marginTop:'8px', display:'flex', gap:'4px', flexWrap:'wrap' }}>{rt.amenities.slice(0,4).map((a:string) => <span key={a} style={{ fontSize:'11px', color:'#6b6760', background:'#f0ede8', padding:'2px 7px', borderRadius:'4px' }}>{a}</span>)}</div>}
                  </div>
      ...
      {step===2 && (
        <div className="anim-fadeup">
          ...
          {nights>0 && <div style={{ background:'#fff', border:'1px solid #e8e6e1', borderRadius:'10px', padding:'14px 18px', marginBottom:'4px', display:'flex', justifyContent:'space-between', alignItems:'center' }}><div style={{ fontSize:'13px', color:'#6b6760' }}>{selectedType?.name} · {nights} nuit{nights>1?'s':''}</div><div style={{ fontFamily:"'DM Serif Display', Georgia, serif", fontSize:'20px', color:'#1a1a1a' }}>€{total.toFixed(2)}</div></div>}
      ...
      {step===3 && (
        <div className="anim-fadeup">
          ...
          <div style={{ marginBottom:'20px' }}><label style={LS}>Demandes spéciales</label><textarea style={{ ...IS, height:'80px', resize:'vertical' }} value={client.requests} onChange={e=>setClient(p=>({...p,requests:e.target.value}))} placeholder="Heure d'arrivée, préférences..." /></div>
          <ConfirmBox>
            <Row label="Chambre" value={selectedType?.name} /><Row label="Arrivée" value={dates.check_in} /><Row label="Départ" value={dates.check_out} /><Row label="Nuits" value={`${nights}`} />
            <div style={{ display:'flex', justifyContent:'space-between', paddingTop:'10px', marginTop:'4px' }}><span style={{ fontWeight:500, color:'#1a1a1a' }}>Total</span><span style={{ fontFamily:"'DM Serif Display', Georgia, serif", fontSize:'20px', color:'#1a1a1a' }}>€{total.toFixed(2)}</span></div>
          </ConfirmBox>
